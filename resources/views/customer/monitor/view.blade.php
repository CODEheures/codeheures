<div class="purchase-title">
    ...
    <div class="right">
        <a href="{{ route('customer.sale.choice') }}" class="btn-yellow2"
           @if(auth()->user()->isDemo) data-intro="Votre crédit est rechargeable en cliquant ici. Pour cela votre compte doit être validé par CODEheures" data-step="5" data-position="top" @endif
        >Recharger mon compte</a>
        <div class="btn-fake reliquat-customer-info"
             @if(auth()->user()->isDemo) data-intro="Ici votre crédit temps restant avec lequel CODEheures peut intervenir" data-step="4" data-position="top" @endif
        >
            Crédit: {{ $totalLeft }}h
        </div>
    </div>
...
@else
    <div class="graph-conso"  @if(auth()->user()->isDemo) data-intro="Ici le graph de
...
@foreach($purchases as $purchase)
    ...
    <div class="purchase" @if(auth()->user()->isDemo)data-intro="Chacun de vos achats apparait ici" data-step="6" data-position="top" @endif>
        <p>
        ...
        <table class="purchase-table" @if(auth()->user()->isDemo) data-intro="Le tableau synthétise les consommations CODEheures justifiées sur cette achat" data-step="7" data-position="top" @endif>
            ...
            <tfoot>
                <tr>
                    <td colspan="4">détails<br /><i class="ion-chevron-up" @if(auth()->user()->isDemo) data-intro="En cliquant ici le tableau s'ouvre et se ferme" data-step="8" data-position="top" @endif></i></td>
                </tr>
            </tfoot>
            <tbody>
...
<div class="purchase" @if(auth()->user()->isDemo) data-intro="Vous trouverez ici la liste des achats de plugins, thèmes etc.. validés sur devis" data-step="10" data-position="top" @endif >
