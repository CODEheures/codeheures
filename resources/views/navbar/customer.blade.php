...
@section('links')
    ...
    <a href="{{ route('customer.quotation.index') }}" {!! ($active == 'quotation') ? 'aria-selected="true"':'aria-selected="false"' !!}>
        <div class="composite-title" @if(auth()->user()->isDemo && $active != 'quotation') data-intro="Ce lien de menu vous permet d'acceder à vos devis que vous pouvez valider en ligne par code SMS"
